...
void playerTurn(CPlayer& player, CDominoes& deck, const std::string& playerName, bool isThisPlayer1, int& head, int& tail) {
    while (true) {
        ...
        turnCV.wait(lock, [&]() { return isPlayer1Turn == isThisPlayer1; });

        {
            ...
            if (player.handEmpty()) {
                ...
                std::cout << "[DEBUG] " << playerName << " has no pieces left. Game Over.\n";
        ...
        if (consecutivePasses == 2) {
            std::cout << "[DEBUG] Both players passed. Game Over.\n";
            gameOver = true;
            return;  // Exit the thread
        }

        // Switch turn
        isPlayer1Turn = !isThisPlayer1;
        lock.unlock();
...
int main() {
    ...
    player1Thread.join();
    std::cout << "[DEBUG] Player 1 thread has finished.\n";
    player2Thread.join();
    std::cout << "[DEBUG] Player 2 thread has finished.\n";

    // Display final results
    if (gameOver) {
        std::cout << "[DEBUG] Game Over flag is set. Displaying results.\n";
        std::cout << "\n============================================================\n";
        ...
        std::cout << "[DEBUG] Game Over flag not set. No results to display.\n";
    }
